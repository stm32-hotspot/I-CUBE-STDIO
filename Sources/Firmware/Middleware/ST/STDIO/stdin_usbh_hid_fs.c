...
* @version        : v 1.0.0
* @brief          : This file implements stdin USBH FS HID channel
******************************************************************************
* @attention
*
...
extern USBH_HandleTypeDef STDIN_USBH_FS_HANDLER; 
...
void stdin_init(void)
{
  do
  {
    do
    {
      USBH_Process(&STDIN_USBH_FS_HANDLER);
    }while(Appli_state != APPLICATION_READY);
  }while(USBH_HID_GetDeviceType(&STDIN_USBH_FS_HANDLER) != HID_KEYBOARD);  
}
...
static uint8_t keyboard_Process(void)
{
  ...
  {
    if(USBH_HID_GetDeviceType(&STDIN_USBH_FS_HANDLER) == HID_KEYBOARD)
    {
      USBH_Keybd_Process(&STDIN_USBH_FS_HANDLER);
      return 0;
    }
