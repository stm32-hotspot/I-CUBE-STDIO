/**
  ******************************************************************************
* @file           : stdin_usbd_cdc_hs.c
* @version        : v 1.0.0
* @brief          : This file implements stdin USBD HS CDC channel
******************************************************************************
* @attention
*
...
extern USBD_HandleTypeDef STDIN_USBD_HS_HANDLER; 
...
int8_t CDC_Receive_HS(uint8_t* Buf, uint32_t *Len)
{
  ...
  USBD_CDC_SetRxBuffer  (&STDIN_USBD_HS_HANDLER, &Buf[0]);
  USBD_CDC_ReceivePacket(&STDIN_USBD_HS_HANDLER);
  return (USBD_OK);
}
